...
class AuthController extends Controller
{
    ...
    public function check_email_and_number(Request $request)
    {
        ...
        if(isset($user))
        {
            return response()->json(['status' => 'error', 'errors' => ['phone_no' => 'Phone No Already Taken.']], 401);
        }
        $user= User::where(["email" => $request->email])->first();
        if (!isset($user)) {
            return response()->json([
    ...
    public function setPwd(Request $request)
    {
     ...
     {
        $user = User::whereId($request->user_id)->with('cust_account_detail', 'cust_basic_detail.banks', 'cust_cnic_detail', 'cust_bank_detail')->first();
        return response()->json([
            'status'       => true,
            'data'         => $user
        ], 200);
     ...
     public function getNotification(Request $request)
     {
        ...
        foreach ($notification as $key => $value) {
            ...
            $data[$key]['title']   = $noti['title'] ?? 'YPay';
            $data[$key]['message'] = $noti['message'];
            $data[$key]['image']   = str_starts_with($noti['image'], 'http')?$noti['image']:env('S3_BUCKET_URL').$noti['image'];
