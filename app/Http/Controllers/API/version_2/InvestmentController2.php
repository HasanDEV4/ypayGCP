...
use Config;
use DB;
class InvestmentController2 extends Controller
{
    ...
    public function getuserinvestmentcount(Request $request)
    {
        ...
        //   })->pluck('amount');
        $conversion_amounts = Conversion::where('user_id',$request->user()->id)->where('fund_id',$request->fund_id)->whereIn('status',[0,1])->where(function($builder){
            $builder->whereDoesntHave('redemption', function ($query){
               $query->whereIn('status',[0,1]);
            })
    ...
    public function addRedemption(Request $request){
        ...
        //   });
          ...
          sendNotification($user->fcm_token, $data, $user_id, 'redemption_pending');
