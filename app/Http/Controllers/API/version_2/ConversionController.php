...
class ConversionController extends Controller
{
	public function save(Request $request){
        ...
        if ($request->type == "investment") {
            $source = Investment::where(['id'=> $request->investment_id, 'user_id' => $user_id, 'status' => 1])->with('redemption', 'fund')->first();
        } else if ($request->type == "conversion") {
            $source = Conversion::where(['id'=> $request->investment_id, 'user_id' => $user_id, 'status' => 1])->with('redemption', 'fund')->first();
        }
        if (!isset($source)) {
            ...
            $source_check_error_message = [
                'investment' => ['We have already received a redemption request for this investment.']
            ];
            return response()->json(['status' => 'error', 'errors' => $source_check_error_message], 422);
        }
        ...
        if ($request->fund_id == $source->fund_id) {
            $fund_check_error_message = [
                'fund' => ['Conversion can not be proccessed into same fund. Please select another fund']
            ];
            return response()->json(['status' => 'error', 'errors' => $fund_check_error_message], 422);
        } else if ($fund->amc_id != $source->fund->amc_id) {
            $fund_check_error_message = [
                'fund' => ["Fund's AMC must be same."]
            ];
            return response()->json(['status' => 'error', 'errors' => $fund_check_error_message], 422);
        }
        ...
        $data = ['message' => Config::get('messages.conversion_request_received'), 'image' => $fund->fund_image];
          sendNotification($user->fcm_token, $data, $user_id, 'investment_pending');
